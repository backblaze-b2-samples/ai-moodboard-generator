...
def public_url(key: str) -> str | None:
    """Durable public URL for a key when B2_PUBLIC_URL_BASE is set, else None."""
    if not settings.b2_public_url_base:
        return None
    return f"{settings.b2_public_url_base}/{quote(key, safe='/')}"


# Backward-compatible alias for internal call sites in this module.
...
def get_s3_client():
    return boto3.client(
        ...
        config=Config(
            ...
            user_agent_extra="b2ai-moodboard",
        ),
    )
...
# --- JSON sidecar / board-object helpers (no database — B2 is the data layer) ---


def put_json(key: str, data: dict) -> None:
    """Write a JSON object to B2. Raises RuntimeError on S3 failure."""
    client = get_s3_client()
...
def get_json(key: str) -> dict | None:
    """Read a JSON object from B2; None if absent. Raises on other S3
    ...
    try:
        ...
        raise RuntimeError(f"B2 object '{key}' is not valid JSON: {e}") from e


def list_prefixes(prefix: str = "") -> list[str]:
    """Immediate sub-prefixes under `prefix` (Delimiter='/'). Used to
    enumerate boards without paging every pin. Raises on S3 failure."""
    client = get_s3_client()
    prefixes: list[str] = []
    kwargs: dict = {
        "Bucket": settings.b2_bucket_name,
    ...
    try:
        ...
        raise RuntimeError(f"B2 list_prefixes failed for '{prefix}': {e}") from e
    return prefixes


def delete_prefix(prefix: str) -> int:
    """Delete every object under `prefix`; returns the count. Scoped only:
    refuses an empty prefix so a whole-bucket wipe is impossible (caller
    passes e.g. `boards/<slug>/`). Raises on S3 failure."""
    if not prefix:
    ...
    try:
        ...
        raise RuntimeError(f"B2 delete_prefix failed for '{prefix}': {e}") from e
    return deleted
