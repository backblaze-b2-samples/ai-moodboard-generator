...
function checkEnv() {
  ...
  if (placeholders.length > 0) {
    fail(
      ...
      "Edit .env and replace placeholders with your real B2 credentials (https://secure.backblaze.com/app_keys.htm?utm_source=github&utm_medium=referral&utm_campaign=ai_artifacts&utm_content=b2ai-moodboard)",
    );
  }

  // Image generation is the core feature; warn (don't fail) if the provider
  // key is absent so `pnpm dev` still boots for browsing existing boards.
  if (!env.OPENAI_API_KEY || env.OPENAI_API_KEY === "your_openai_api_key") {
    warn(
      "OPENAI_API_KEY is not set — /generate will return an error until it is",
      "Add OPENAI_API_KEY to .env (https://platform.openai.com/api-keys). Generation routes through Genblaze + OpenAI gpt-image-1.",
    );
  }
}
